...
ALIAS_COLUMNAS = {
    "nombre": ("nombre", "nombre y apellido", "nombres", "persona",
               "contacto", "inscrito"),
...
def leer_base(ruta):
    ...
    if indice_encabezado is None:
        ...
        return [{"correo": f[mejor] if mejor < len(f) else "",
                 "nombre": "", "institucion": "", "ciudad": "", "segmento": ""}
                for f in filas]
    ...
    for fila in filas[indice_encabezado + 1:]:
        ...
        for campo in ("nombre", "institucion", "ciudad", "segmento"):
...
def preparar_contactos(crudos, excluidos=(), excluir_sin_institucion=False):
    ...
    for registro in crudos:
        ...
        buenos.append({
            "correo": correo,
            "nombre": arreglar_mayusculas(registro.get("nombre", "")),
...
def bloques_saludo(contacto):
    """
    Saludo personal. Este correo no va a una institucion en frio: va a
    alguien que ya se inscribio y dio su nombre, asi que se le habla
    por el nombre propio.

        Buen dia, Rafael Leonardo:

    Si no hay nombre, queda un saludo neutro que no suena a plantilla.
    """
    nombre = (contacto.get("nombre") or "").strip()
    if nombre:
        # Dos nombres de pila bastan; el apellido completo suena a oficio.
        partes = nombre.split()
        trato = " ".join(partes[:2]) if len(partes) > 2 else partes[0]
        texto = "Buen día, %s:" % trato
    else:
        texto = "Buen día:"
    return texto, _html.escape(texto)
...
def construir_mensaje(contacto, configuracion, plantillas, para=None):
    ...
    # SIN List-Unsubscribe a proposito: esto no es publicidad, es la
    # confirmacion de una inscripcion que la persona pidio. Esa cabecera
...
def orden_enviar(args, configuracion):
    # Seguro: sin el enlace del formulario este correo no sirve de nada.
    enlace = configuracion["mensaje"]["enlace_inscripcion"].strip()
    if not enlace.lower().startswith("http"):
        print()
        print("  ALTO. Falta el enlace del formulario.")
        print()
        print("  Abra config.ini, busque la linea que dice:")
        print("      enlace_inscripcion = %s" % enlace)
        print("  y reemplace eso por el enlace de su formulario de Google,")
        print("  el que empieza en https://forms.gle/")
        print()
        print("  No se envio ningun correo.")
        return 1
